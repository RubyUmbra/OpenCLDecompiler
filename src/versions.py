...
def find_max_and_prev_versions(curr_node):
    for reg in curr_node.state.registers:
        ...
        for parent in curr_node.parent:
            if parent.state.registers.get(reg) is not None and parent.state.registers[reg].version is not None:
    ...
    if (
        ...
        if decompiler_data.checked_variables.get(check_version) is not None:
            if curr_node.state.registers[first_reg].val.find(val_reg) != -1:
                curr_node.state.registers[first_reg].register_content._value = curr_node.state.registers[
                    first_reg
                ].val.replace(val_reg, decompiler_data.checked_variables[check_version])  # pylint: disable=W0212
