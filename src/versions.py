...
def find_max_and_prev_versions(curr_node):
    for reg in curr_node.state.registers:
        ...
        for parent in curr_node.parent:
            if (
                parent.state.registers.get(reg) is not None
    ...
    for prev in prev_versions_of_reg:
        if decompiler_data.checked_variables.get(prev):
            ...
            decompiler_data.variables = {
                k: v.replace(old_var, variable) for k, v in decompiler_data.variables.items()
            }
    curr_node.state.registers[reg].register_content._value = variable  # pylint: disable=W0212
    if curr_node.state.registers[reg].type in [
...
def check_for_use_new_version_in_one_instruction(curr_node):
    for num_of_reg in range(1, len(curr_node.instruction)):
        ...
        if (
            ...
            parent_register = curr_node.parent[0].state.registers.get(register)
...
def update_value_for_reg(first_reg, curr_node):
    for child in curr_node.children:
        if (
            len(child.parent) < 2
            and curr_node.state.registers[first_reg].version
            == child.state.registers[first_reg].version
        ):
...
def update_val_from_changes(curr_node, register, changes, check_version, num_of_reg, first_reg):
    ...
    if (
        curr_node.state.registers.get(register) is not None
        ...
        if re.match(r"(flat|global)_store", instruction[0]):
            ...
            if first_reg != register:
                node_registers[register].register_content._value = node_registers[
                    register
                ].val.replace(  # pylint: disable=W0212
    ...
    if (
        ...
        if decompiler_data.checked_variables.get(check_version) is not None:
            if curr_node.state.registers[first_reg].val.find(val_reg) != -1:
                curr_node.state.registers[
                    first_reg
                ].register_content._value = curr_node.state.registers[first_reg].val.replace(
                    val_reg, decompiler_data.checked_variables[check_version]
                )  # pylint: disable=W0212
            elif re.match(r"(flat|global)_store", instruction[0]):
                curr_node.state.registers[
                    register
                ].register_content._value = curr_node.state.registers[register].val.replace(
                    val_reg, decompiler_data.checked_variables[check_version]
                )  # pylint: disable=W0212
        ...
        elif curr_node.state.registers[first_reg].val.find(val_reg) != -1 and first_reg in [
            "vcc",
            "scc",
            "exec",
        ]:
            ...
            curr_node.state.registers[register].register_content._value = curr_node.state.registers[
                register
            ].val.replace(val_reg, decompiler_data.variables[check_version])  # pylint: disable=W0212
        ...
        if (
            ...
            if register == first_reg:
                if curr_node.parent[0].state.registers.get(register) is not None:
                    ...
                    continue
            changes = update_val_from_changes(
                curr_node, register, changes, check_version, num_of_reg, first_reg
            )
            update_val_from_checked_variables(
                curr_node, register, check_version, first_reg, changes
            )
    ...
    while queue:
        ...
        if curr_node not in visited:
            ...
            if len(curr_node.parent) < 2:
                instruction = curr_node.instruction
                if (instruction != "c_branch" or curr_node in decompiler_data.back_edges) and len(
                    instruction
                ) > 1:
