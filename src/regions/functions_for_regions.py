            ...
            if curr_node in decompiler_data.loops or curr_node in decompiler_data.back_edges:
                ...
                for c_p in curr_node.parent:
                    if c_p in visited:
                        ...
                        region.add_parent(parent)
            elif len(curr_node.parent) == 1 and (
                len(curr_node.children) == 1 or len(curr_node.children) == 0
            ):
...
def process_if_statement_region(curr_region):
    ...
    child0 = (
        ...
        if len(curr_region.children[0].parent) == 1
        else curr_region.children[1]
    )
    child1 = (
        curr_region.children[1]
        if len(curr_region.children[1].parent) > 1
        else curr_region.children[0]
    )
    ...
    if reg_versions_in_instruction.get(register_version):
        ...
        for version in instruction_version_list:
            ...
            if version != instruction_register_version:
                ...
                make_var_for_loop(
                    change_node, instruction_register, register_version, prev_register_version
                )
...
def process_loop(region_start, region_end):
    ...
    while curr_node != region_end.start:
        ...
        list_of_reg_nums = (
            list_of_reg_nums if len(curr_node.instruction) == 1 else list_of_reg_nums + [1]
        )
        ...
        for num_of_register in list_of_reg_nums:
            ...
            if curr_node.state.registers.get(register):
                if "cmp" not in curr_node.instruction[0] and not re.match(
                    ...
                    if num_of_register > 1 and register != first_reg:
                        ...
                        check_changes_in_reg(
                            register, reg_versions_in_instruction, curr_node, reg_version_node
                        )
                else:
                    check_changes_in_reg(
                        register, reg_versions_in_instruction, curr_node, reg_version_node
            ...
            if (
                ...
                if first_reg_prev_version in used_versions_of_registers:
                    make_var_for_loop(
                        curr_node, register, first_reg_version, first_reg_prev_version
                    )
    ...
    while start_region.children[0] != after_region_end:
        ...
        if curr_region not in visited:
            ...
            if (
                ...
                curr_region.type = (
                    RegionType.BREAK_REGION
                )  # надо отдельно написать на return и обрезание на break
                next_region = (
                    curr_region.children[0]
                    if curr_region.children[1] == after_region_end
                    else curr_region.children[1]
                )
...
def get_one_loop_region(q_loops, curr_region, start_region, region_start, region_end):
    while q_loops:
        ...
        if loop_region.type == RegionType.BACK_EDGE:
            ...
            join_regions(
                loop_region.parent[0], loop_region, loop_region.children[0]
            )  # not good enough
