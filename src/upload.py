...
def upload_kernel_param(state, offset, to_registers):
    ...
    while start <= end:
        ...
        if content.get_size() <= 4:
            state.registers[f"s{start}"] = Register(
                integrity=Integrity.ENTIRE, register_content=content
            )
            ...
            if start + 1 > end:
                ...
                break
            state.registers[f"s{start}"] = Register(
                integrity=Integrity.LOW_PART, register_content=content
            )
            decompiler_data.make_version(state, f"s{start}")
            state.registers[f"s{start + 1}"] = Register(
                integrity=Integrity.HIGH_PART, register_content=content
            )
...
def upload_by_offset(
    ...
    while True:
        ...
        if (
            state.registers[curr_to_register].register_content.get_size()
            + register_content.get_size()
            < state.registers[curr_to_register].get_size()
        ) or (
            state.registers[curr_to_register].register_content.get_size()
            + register_content.get_size()
            == state.registers[curr_to_register].get_size()
            and state.registers[curr_to_register].register_content.get_count() != 0
        ):
            ...
            if (
                state.registers[curr_to_register].register_content.get_size() == 0
                and register_content.get_size() == state.registers[curr_to_register].get_size() * 2
            ):
                state.registers[curr_to_register] = Register(
