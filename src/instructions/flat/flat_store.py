...
def prepare_vector_type_output(from_registers, vdata, to_registers, node):
    new_vector = [
        node.state.registers[reg].val for reg in check_and_split_regs_range_to_full_list(vdata)
    ]
    ...
    if is_same_name(new_vector) and (
        not is_vector_type(from_type)
        and to_type[:-1] == make_opencl_type(from_type)
        ...
        output_string = (
            "(" + node.state.registers[to_registers].data_type + ")(" + ", ".join(new_vector) + ")"
        )
...
class FlatStore(BaseInstruction):
    ...
    def to_fill_node(self):
        if self.suffix in [
            ...
            for self.from_registers in check_and_split_regs_range_to_full_list(self.vdata)[
                :suffix_size
            ]:
                ...
                # TODO: Сделать присвоение в пары
                    ...
                    if (
                        self.node.state.registers[self.from_registers].data_type is not None
                        ...
                        self.decompiler_data.names_of_vars[
                            self.node.state.registers[self.from_registers].val
                        ] = self.node.state.registers[self.to_registers].data_type
                    else:
                        if (
                            str(self.node.state.registers[self.from_registers].data_type)
                            not in self.node.state.registers[self.to_registers].data_type
                            and not is_vector_type(
                            ...
                            if val not in self.decompiler_data.names_of_vars:
                                ...
                                self.decompiler_data.names_of_vars[val] = self.node.state.registers[
                                    self.from_registers
                                ].data_type
                            else:
    ...
    def to_print(self):
        if self.suffix in ["dword", "dwordx2", "dwordx4", "byte", "short", "b32", "b64", "b8"]:
            ...
            if is_sgpr_range(self.inst_offset):
                ...
                if (
                    self.node.state.registers[offset_reg].get_type()
            ...
            if self.inst_offset == "inst_offset:4":
                ...
                if self.decompiler_data.names_of_vars.get(var):
                    var = (
                        "*("
                        + make_opencl_type(self.decompiler_data.names_of_vars[var])
                        + "*)("
                        + var
                        + ")"
                    )
            ...
            if self.node.state.registers.get(self.from_registers):
                if self.node.state.registers[
                    self.from_registers
                ].val == "0" and self.node.state.registers.get(self.from_registers_1):
                    ...
                    if is_vector_type(self.node.state.registers[self.to_registers].data_type):
                        ...
                        self.output_string = self.node.state.registers[
                            self.from_registers
                        ].get_value()
            else:
                self.output_string = self.decompiler_data.initial_state.registers[
                    self.from_registers
                ].val
