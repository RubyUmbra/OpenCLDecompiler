...
class GlobalLoad(FlatLoad):
    ...
    def to_fill_node(self):
        if (
            ...
            and self.from_registers in self.node.state.registers
            and self.node.state.registers[self.from_registers].type == RegisterType.DISPATCH_POINTER
        ...
        if self.suffix in ["b32", "u16", "u8"]:
            if (
                self.node.state.registers[self.from_registers].type
                == RegisterType.ARGUMENTS_POINTER
            ):
