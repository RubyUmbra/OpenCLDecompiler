...
class FlatLoad(BaseInstruction):
    ...
    def to_fill_node(self):  # нужно ли здесь делать self.node
        if self.suffix in ["dword", "dwordx2", "dwordx4"]:
            ...
            self.decompiler_data.var_value[variable] = self.node.state.registers[
                self.from_registers
            ].val
            register_type = (
                RegisterType.KERNEL_ARGUMENT_PTR
                if self.node.state.registers[self.from_registers].type
            ...
            while True:
                ...
                self.node = set_reg_value(
                    self.node, reg_val, to_now, [], data_type, reg_type=register_type
                )
                if to_now == self.end_to_registers:
                    break
                to_now = get_next_reg(to_now)
            self.decompiler_data.make_var(
                self.node.state.registers[self.start_to_registers].version,
                self.node.state.registers[self.start_to_registers].val,
                data_type,
            )
            return self.node
    ...
    def to_print(self):
        if self.suffix in ["dword", "dwordx2", "dwordx4"]:
            ...
            if " + " in output:
                ...
                output = (
                    "*("
                    + make_opencl_type(self.decompiler_data.names_of_vars[output])
                    + "*)("
                    + output
                    + ")"
                )
