...
class SLshl(BaseInstruction):
    ...
    def to_print_unresolved(self):
        if self.suffix == "b32":
            self.decompiler_data.write(
                self.sdst + " = " + self.ssrc0 + " << (" + self.ssrc1 + " & 31) // s_lshl_b32\n"
            )
            self.decompiler_data.write("scc = " + self.sdst + "!= 0\n")
            return self.node
        if self.suffix == "b64":
            self.decompiler_data.write(
                self.sdst + " = " + self.ssrc0 + " << (" + self.ssrc1 + " & 63) // s_lshl_b64\n"
            )
            self.decompiler_data.write("scc = " + self.sdst + "!= 0\n")
            return self.node
    ...
    def to_fill_node(self):
        ...
        if self.suffix == "b64":
            ...
            new_value1 = make_op(
                self.node, end_from_register, str(pow(2, int(self.ssrc1))), "*", suffix=self.suffix
            )
