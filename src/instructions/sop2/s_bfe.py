...
class SBfe(BaseInstruction):
    ...
    def to_print_unresolved(self):
        ...
        if self.suffix == "u32":
            self.decompiler_data.write(
                "uchar " + shift + " = " + self.ssrc1 + " & 31 // s_bfe_u32\n"
            )
        ...
        if self.suffix == "i32":
            self.decompiler_data.write(
                "uchar " + shift + " = " + self.ssrc1 + " & 31 // s_bfe_i32\n"
            )
            self.decompiler_data.write("uchar " + length + " = (" + self.ssrc1 + ">>16) & 07xf\n")
            self.decompiler_data.write("if (" + length + "==0)\n")
            self.decompiler_data.write(tab + self.sdst + " = 0\n")
            ...
            self.decompiler_data.write("else\n")
            self.decompiler_data.write(
                tab + self.sdst + " = (int)" + self.ssrc0 + " >> " + shift + "\n"
            )
            self.decompiler_data.write("scc = " + self.sdst + " != 0\n")
            self.decompiler_data.number_of_length += 1
            self.decompiler_data.number_of_shift += 1
    ...
    def to_fill_node(self):
        ...
        if self.suffix == "u32":
            ...
            elif self.decompiler_data.bfe_offsets.get(
                (self.node.state.registers[self.ssrc0].val, self.ssrc1)
            ):
                new_value = self.decompiler_data.bfe_offsets[
                    self.node.state.registers[self.ssrc0].val, self.ssrc1
                ]
            ...
            return set_reg_value(
                self.node, new_value, self.sdst, [], self.suffix, reg_type=reg_type
            )
        if self.suffix == "i32":
            if self.decompiler_data.bfe_offsets.get(
                (self.node.state.registers[self.ssrc0].val, self.ssrc1)
            ):
                new_value = self.decompiler_data.bfe_offsets[
                    self.node.state.registers[self.ssrc0].val, self.ssrc1
