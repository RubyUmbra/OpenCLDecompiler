    ...
    def to_print_unresolved(self):
        if self.suffix == "i32":
            self.decompiler_data.write(
                self.sdst + " = " + self.ssrc0 + " * " + self.ssrc1 + " // s_mul_i32\n"
            )
            return self.node
    ...
    def to_fill_node(self):
        if self.suffix == "i32":
            ...
            if ssrc0_reg and ssrc1_reg:
                if (
                    self.node.state.registers[self.ssrc0].type == RegisterType.LOCAL_SIZE_X
                    ...
                    self.node.state.registers[self.ssrc0].type == RegisterType.LOCAL_SIZE_Y
                    ...
                    self.node.state.registers[self.ssrc0].type == RegisterType.LOCAL_SIZE_Z
