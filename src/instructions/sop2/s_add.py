    ...
    def to_fill_node(self):
        if self.suffix in {"u32", "i32"}:
            if self.decompiler_data.is_rdna3:
                try:
                    new_reg = (
                        self.node.state.registers[self.ssrc0]
                        + self.node.state.registers[self.ssrc1]
                    )
            ...
            new_value = make_op(
                self.node, self.ssrc0, self.ssrc1, "+", "(ulong)", "(ulong)", suffix=self.suffix
            )
            ...
            if (
                self.ssrc1.isdigit()
                and ssrc0_reg
                and self.node.state.registers[self.ssrc0].type == RegisterType.ARGUMENTS_POINTER
            ):
                ...
                if src_types in _instruction_special_cases:
                    ...
                    if self.node.state.registers[self.ssrc0].data_type in [
                        "u32",
                        "i32",
                        "gi32",
                        "gu32",
                        ...
                        new_value = make_op(self.node, self.ssrc1, "4", "/", suffix=self.suffix)
                        new_value = make_op(
                            self.node, self.ssrc0, new_value, "+", suffix=self.suffix
                        )
                ...
                if (
                    self.node.state.registers[self.ssrc0].type
