...
class VCvt(BaseInstruction):
    ...
    def to_print_unresolved(self):
        ...
        if self.suffix == "f64_i32":
            self.decompiler_data.write(
                self.vdst + " = (double)(int)" + self.src0 + " // v_cvt_f64_i32\n"
            )
            return self.node
        if self.suffix == "f64_u32":
            self.decompiler_data.write(
                self.vdst + " = (double)(uint)" + self.src0 + " // v_cvt_f64_u32\n"
            )
            return self.node
        if self.suffix == "i32_f64":
            self.decompiler_data.write(
                self.vdst + " = (int)(double)" + self.src0 + " // v_cvt_i32_f64\n"
            )
            return self.node
        if self.suffix == "u32_f64":
            self.decompiler_data.write(
                self.vdst + " = (uint)(double)" + self.src0 + " // v_cvt_u32_f64\n"
            )
            return self.node
        if self.suffix == "u32_f32":
            ...
            self.decompiler_data.write(
                tab
                + self.vdst
                + " = (int)min(convert_int_rtz(as_float("
                + self.src0
                + ")), 4294967295.0)\n"
            )
            return self.node
        if self.suffix == "i32_f32":
            self.decompiler_data.write(
                self.vdst + " = (int)(float)" + self.src0 + " // v_cvt_i32_f32\n"
            )
            return self.node
        if self.suffix == "f32_i32":
            self.decompiler_data.write(
                self.vdst + " = (float)(int)" + self.src0 + " // v_cvt_f32_i32\n"
            )
            return self.node
    ...
    def to_fill_node(self):
        if self.suffix in [
            ...
            if (
                self.src0 in self.node.state.registers
                and self.node.state.registers[self.src0].type == RegisterType.DIVISION_PT2
            ):
            ...
            new_value = (
                "("
                + make_opencl_type(self.suffix[:3])
                + ")"
                + self.node.state.registers[self.from_registers].val
            )
            reg_type = self.node.state.registers[self.from_registers].type
            return set_reg_value(
                self.node, new_value, self.to_registers, [], asm_type, reg_type=reg_type
            )
