    ...
    def to_print_unresolved(self):
        if self.suffix == "f32":
            self.decompiler_data.write(
                f"{self.vdst} = (float){self.src0} * (float){self.src1} // {self.name}\n"
            )
            return self.node
    ...
    def to_fill_node(self):
        if self.suffix == "f32":
            if (
                ...
                and self.node.state.registers[self.src1].type == RegisterType.DIVISION_RECIPROCAL
            ...
            new_value = make_op(
                self.node, self.src0, self.src1, "*", "(float)", "(float)", suffix=self.suffix
            )
            return set_reg_value(
                self.node,
                new_value,
        ...
        if self.suffix == "i32_i24":
            reg_entire = self.node.state.registers[self.src1].integrity
            new_value = make_op(
                self.node, self.src0, self.src1, "*", "(int)", "(int)", suffix=self.suffix
            )
            return set_reg_value(
                self.node,
                new_value,
