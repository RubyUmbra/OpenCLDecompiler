...
class VAdd(BaseInstruction):
    ...
    def to_print_unresolved(self):
        if self.suffix == "u32":
            ...
            self.decompiler_data.write(
                self.vdst + " = CLAMP ? min(" + temp + ", 0xffffffff) : " + temp + "\n"
            )
    ...
    def to_fill_node(self):
        if self.suffix == "u32":
            if self.decompiler_data.is_rdna3:
                if (
                    ...
                    and self.decompiler_data.type_params.get(
                    ...
                    new_value = make_op(
                        self.node, self.src1, str(data_size), "/", suffix=self.suffix
                    )
            ...
            if src0_reg and src1_reg:
                ...
                if (
                    self.node.state.registers[self.src0].type
                    == RegisterType.WORK_GROUP_ID_X_LOCAL_SIZE_OFFSET
                    ...
                    self.node.state.registers[self.src0].type
                    == RegisterType.WORK_GROUP_ID_Y_LOCAL_SIZE_OFFSET
                    ...
                    self.node.state.registers[self.src0].type
                    == RegisterType.WORK_GROUP_ID_Z_LOCAL_SIZE_OFFSET
                    ...
                    and self.node.state.registers[self.src0].type
                    == RegisterType.WORK_GROUP_ID_Z_WORK_ITEM_ID
                ):
                    ...
                    == RegisterType.WORK_GROUP_ID_X_LOCAL_SIZE
                    and self.node.state.registers[self.src1].type == RegisterType.WORK_ITEM_ID_X
                    or self.node.state.registers[self.src1].type
                    == RegisterType.WORK_GROUP_ID_X_LOCAL_SIZE
                ...
                if src0_reg:
                    ...
                    if (
                        ...
                        new_value = make_op(
                            self.node, self.src0, new_value, "+", suffix=self.suffix
                        )
                if src1_reg:
                    reg_type = self.node.state.registers[self.src1].type
            return set_reg_value(
        ...
        if self.suffix == "f64":
            ...
            if (
                self.node.state.registers[start_from_src1].val
                == self.node.state.registers[start_from_src0].val
            ):
