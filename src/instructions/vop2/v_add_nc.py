    ...
    def to_print_unresolved(self):
        if self.suffix in ["u16", "u32"]:
            self.decompiler_data.write(
                f"{self.vdst} = {self.src0} + {self.src1} // {self.instruction[0]}\n"
            )
            return self.node
    ...
    def to_fill_node(self):
        if self.suffix in ["u16", "u32"]:
            if self.decompiler_data.is_rdna3:
                try:
                    new_reg = (
                        self.node.state.registers[self.src0] + self.node.state.registers[self.src1]
                    )
                    return set_reg(
                        node=self.node,
                        to_reg=self.vdst,
            ...
            if (
                ...
                and self.node.state.registers[self.src1].type
            ...
            new_value = make_op(
                self.node, self.src0, self.src1, "+", "(ulong)", "(ulong)", suffix=self.suffix
            )
