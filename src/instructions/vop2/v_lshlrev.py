    ...
    def to_print_unresolved(self):
        if self.suffix in ["b16", "b32", "b64"]:
            ...
            self.decompiler_data.write(
                f"{self.vdst} = {self.src1} << ({self.src0}&{num}) // {self.instruction[0]}\n"
            )
            return self.node
    ...
    def to_fill_node(self):
        if self.suffix in ["b16", "b32"]:
            if is_reg(self.src1) and self.node.state.registers[self.src1].val == "0":
                ...
                new_value = make_op(
                    self.node, self.src1, str(pow(2, int(self.src0))), "*", suffix=self.suffix
                )
        ...
        if self.suffix == "b64":
            ...
            new_value0 = make_op(
                self.node, start_from_register, str(pow(2, int(self.src0))), "*", suffix=self.suffix
            )
            new_value1 = make_op(
                self.node, end_from_register, str(pow(2, int(self.src0))), "*", suffix=self.suffix
            )
