    ...
    def to_print_unresolved(self):
        ...
        if self.suffix == "f64":
            ...
            self.decompiler_data.write(
                "double " + sf0 + " = as_double(" + self.src0 + ") // v_div_fixup_f64\n"
            )
            ...
            self.decompiler_data.write(
                tab + self.vdst + " = INFINITY * sign(" + sf1 + ") * sign(" + sf2 + ")\n"
            )
            self.decompiler_data.write("else if (abs(" + sf1 + ") == INFINITY)\n")
            self.decompiler_data.write(
                tab + self.vdst + " = sign(" + sf1 + ") * sign(" + sf2 + ") >= 0 ? 0.0 : -0.0\n"
            )
            self.decompiler_data.write("else if (isnan(" + sf0 + "))\n")
            self.decompiler_data.write(
                tab + self.vdst + " = sign(" + sf1 + ") * sign(" + sf2 + ") * INFINITY\n"
            )
