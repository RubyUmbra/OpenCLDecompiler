...
class AmdGpuDisParser:
    ...
    def parse(self, text: str) -> Optional[tuple[ParseObject, str]]:
        ...
        config_parser = OneOrMoreParser(AmdGpuDisConfigsParser() | IgnoreParser(LineParser(AnyParser())))
                    ...
                    for obj in result.obj:
                        yield (
                            ...
                            ConfigData(
                                ...
                                size_of_work_groups=[
                                    int(wg_item_size, 16)
                                    for wg_item_size in self._config[obj.name][".reqd_workgroup_size"]
                                ],
                                local_size=None,
                                arguments=[
                                    self._make_argument(idx, arg)
                                    for idx, arg in enumerate(list(self._config[obj.name][".args"]))
                                ],
